...
- `ReportSccCompletion` 是 shard 内部 RPC，不暴露给 client；用于 speculative phase 结束后交换各 shard 观察到的 failed tx index 集合。最终 deterministic reorder fallback 的重执行序列由全体 failed set 的并集推导。
- `DumpState` 是 test/debug RPC；SCC 模式下额外返回每个 batch 的 deterministic reorder summary，供 checker 比较各 shard 是否采用同一重排顺序。
...
14. speculative phase 结束后，coordinator 计算本 shard 的 failed tx index 集合，并通过 `ReportSccCompletion` 与所有 shard 交换。
15. coordinator 对全体 shard 的 failed set 取并集，得到确定性的 `global_failed_indices`。
16. coordinator 按原 batch index 顺序安装所有不在 `global_failed_indices` 中的本 shard local delta。
17. coordinator 按 `global_failed_indices` 的升序构造 fallback 重执行序列，并对这些事务执行 Calvin deterministic-locking fallback。
...
### 8.2 Failure set exchange

完整 fallback 要求所有 shard 使用同一个 failed set。只在本地推导 failed set 不够，因为同一个事务的推测失败会出现只被部分 participant 观察到的情况，而其它 shard 会把该事务视为本地 `NoOp`。

speculative phase 结束后，每个 shard 维护：

- `local_failed_indices`：本 shard commit sequence 中状态为 `Failed` 的 tx index。
- `peer_failed_indices`：每个 peer shard 上报的 failed tx index 集合。
- `global_failed_indices`：所有 shard failed set 的并集，按 batch index 升序排列。

交换流程：

1. 本 shard 所有 DAG vertex terminal 后，coordinator 计算 `local_failed_indices`。
2. coordinator 将本地 failed set 写入 registry，并向所有 peer shards 并发调用 `ReportSccCompletion`。
3. `ReportSccCompletion` handler 只负责把 failed set 投递到本地 registry；如果 report 早于 batch executor 等待，也必须能暂存。
4. batch executor 等到收齐全体 `shard_count` 个 completion reports。
5. coordinator 计算 `global_failed_indices = union(peer.failed_tx_indices)`。

约束：

- 如果本 shard 已经为某个 tx 发布 speculative `TxResult::Ok`，该 tx 不得出现在最终 `global_failed_indices` 中。出现该情况说明参与分片对同一 full read set 的执行或条件检查不一致，是 internal error。
- 如果某个 tx 出现在 `global_failed_indices` 中，即使本 shard 本地 slot 是 `NoOp`，也必须把该 tx 放入 fallback 重执行序列；该 `NoOp` 只表示本 shard speculative phase 中没有本地 delta，不表示全局成功。
...
pub struct SccCompletionReportRegistry {
    // keyed by batch_id, stores early and in-flight reports
}
```

这个 barrier 是 SCC 内部协议，不改变 Sequencer 的 batch dispatch 模型。Sequencer 仍然只等待每个 shard 的 `ExecuteBatchResponse`。

### 8.3 Deterministic reorder fallback

每个 shard 在收齐 `global_failed_indices` 后，按同一确定性规则完成 batch：

1. 按原 batch index 升序扫描 commit sequence。
2. 对不在 `global_failed_indices` 中的 slot：
   - `Delta(delta)`：安装到本 shard store。
   - `NoOp`：跳过。
   - `Failed`：如果该 index 不在 `global_failed_indices` 中，说明 failed set exchange 出错，是 internal error。
3. 对在 `global_failed_indices` 中的 slot，不安装 speculative delta。
4. 用 `global_failed_indices` 按升序构造 fallback 重执行序列。
...
checker 不重新实现 SCC 调度来猜测最终串行化顺序。SCC shard 必须通过 `DumpState.scc_reorders` 暴露每个 batch 的：

- `speculative_success_indices`
- `fallback_indices`

checker 先检查所有 shard 对同一个 `batch_id` 报告的两个序列完全一致，再按该顺序做 reorder-aware reference execution。
...
- `completion_report_unions_failed_indices_for_reorder_fallback`
...
- proto 增加 shard 内部 `ReportSccCompletion` RPC。
- mailbox key 增加 phase。
- Calvin worker 使用 `ReadPhase::Calvin`。
- 增加 `SccCompletionReportRegistry`，支持 early completion report 暂存。
...
- failed slot 传播。
- 全 shard completion report 和 failed set union。
